...
| D-016 | **Slice N is GitHub issue N+1.** GitHub issue numbers start at 1, so `CS-0` is issue #1 and `CS-32` is issue #33. Issue titles carry the slice ID; branches carry the GitHub number, per Playbook §2. The offset is constant. | This slice (CS-0) | 25 Aug 2026 |
